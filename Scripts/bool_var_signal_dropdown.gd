...
func replace_self_with_bool_constructor() -> void:
	var parent = get_parent()
	if not (parent is Control):
		return
	var instance = load(bool_constructor_path).instantiate()
...
func refresh_trigger_list() -> void:
	...
	for trigger_node in triggers_dict.keys():
		add_item(triggers_dict[trigger_node][3], 4)

func _on_item_selected(index: int) -> void:
	match index:
		0: # New logic gate
			replace_self_with_bool_constructor()
		1: # True
			target_trigger = "true"
		2: # False
