...
@onready var level_source_tab_bar: TabBar = level_source_tab_container.get_tab_bar()
# Campaign Play
...
var sorted_level_array: Array = [] # Format is level_id at position index

func _ready() -> void:
	level_source_tab_bar.set_tab_disabled(1, true)
	level_source_tab_container.current_tab = 0
	
	Global.steam_connection_established.connect(_on_steam_connected)
	if Global.is_steam_connected: # Redundancy incase signal function connects after steam connection
		_on_steam_connected()
...
func select_next_level_id() -> bool:
	if selected_level_id == "tutorial":
		selected_level_id = sorted_level_array[1]
		return true
	
	if not level_cards.has(selected_level_id):
		print("Level " + selected_level_id + " not in level cards")
		return false
	
	var current_level_index: int = level_cards.get(selected_level_id)[1]
	
	if sorted_level_array[current_level_index] != selected_level_id:
		push_error("Level sorting error. Index " + str(current_level_index) + " missmatch for level id " + selected_level_id)
		return false
	
	if (sorted_level_array.size() - 1) < (current_level_index + 1):
		print("Level " + selected_level_id + " at index " + str(current_level_index) + " is the last level")
		return false
	
	selected_level_id = sorted_level_array[current_level_index + 1]
...
func _on_level_selected() -> void:
	var level_card_info: Array = []
	
	if selected_level_id == "tutorial":
		level_card_info = [$PanelContainer/MarginContainer/TabContainer/Campaign_Levels/VBoxContainer/ScrollContainer/MarginContainer/GridContainer/Tutorial_Card,
		-1,
		"Tutorial",
		[false, false, false],
		false,
		"Learn to play at your own pace in a series of guided steps"]
		campaign_level_rating_info_tab.text = ""
	else:
		level_card_info = level_cards.get(selected_level_id)
...
func populate_campaign_level_cards(clear_previous: bool = true) -> void:
	...
	for file_name in local_level_files:
		...
		var level_card_index: int = level_cards.size()
		level_cards[file_name] = [new_level_card, level_card_index, level_data["level_name"], level_data["completion_rating"], false, level_data["level_description"]]
		# Format is level_id: [node, index, level_name, completion_rating, is_locked, level_description]
		sorted_level_array.resize(level_card_index + 1)
		sorted_level_array[level_card_index] = file_name
		
		level_data = {} # Clears the loaded file from RAM

func update_rating_on_selected_level_card(new_level_rating: Array) -> void:
	if level_cards.has(selected_level_id):
...
func _on_steam_connected() -> void:
	level_source_tab_bar.set_tab_disabled(1, false)
...
func _on_campaign_play_pressed() -> void:
	if selected_level_id == "tutorial":
		Global.world_scene.button_signal("tutorial")
	else:
